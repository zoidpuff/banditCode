...
avargedData <- mutate(avargedData, State = ifelse(MouseType == "DTR+", paste0(State,"DTRplus"), State)) %>%
                filter(MouseType != "GCAMP") %>%
...
plotDifferencesTimeProgressionComparison <- function(avargedData,comparisonVec,varibleToCompare) {

                datas <- filter(avargedData, State %in% comparisonVec)
                datas$State <- factor(datas$State,levels = comparisonVec)
                datas$Mouse <- as.character(datas$Mouse)
                datas <- datas %>% mutate(TimeSegment = cut(MidTimePoint, breaks = 3))

                datasFirst <- filter(datas, State == comparisonVec[1])	
                datasSecond <- filter(datas, State == comparisonVec[2])

                forumlaString <- paste0("`",varibleToCompare,"`", "~ MidTimePoint + State")
                
                comblmnosum <- lm(forumlaString,datas)
                comblm <-summary(comblmnosum)
                ...
                ggpubr::stat_regline_equation(data = datas,
                                                mapping = aes_string(x = "MidTimePoint", y = paste0("`",varibleToCompare,"`"))) + 
                ggpubr::stat_cor(data = datas,
                                                mapping = aes_string(x = "MidTimePoint", y = paste0("`",varibleToCompare,"`")),label.x.npc = "middle") + 
                xlab("Time (s)") +
                guides(color = "none") + 
                ggtitle(paste0(varibleToCompare, " : ", comparisonVec[1]," vs. ",comparisonVec[2]),
                        subtitle = paste0("Significance P-value of Linear Model     = ",formatC(getpval(comblmnosum), format = "e", digits = 3),"     (",varibleToCompare," ~ Time + State )",
                                        "\nSignificance P-value of Time Coefficent  = ", formatC(comblm$coefficients[2,4], format = "e", digits = 3),
                                        "\nSignificance P-value of State Coefficent = ", formatC(comblm$coefficients[3,4], format = "e", digits = 3),
                                        "\nRatio of Slopes: ",abs(round(ratioOfCoeffcients,3))))
                return(p)

ggsave("file.png",p,width = 10,height = 7)

}

plotFolder <- "plots/ComaparingStatesWithArchs"

avargedData <- as.data.frame(avargedData)
avargedData$Mouse <- as.character(avargedData$Mouse)
...
for(i in 1:length(featuresToCompare)) {
        for(j in 1:length(contrastList)) {
                a <- plotDifferencesTimeProgressionComparison(avargedDataWarchalphas,contrastList[[j]],featuresToCompare[i])
